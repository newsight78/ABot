...
void CABotDlg::LoadSystemFile()
{
	...
	AddMessage("     종목단 최대 투자 허용 금액 [%s]만원.", strBuf);
	...
	ReadFromIniFile_String(m_strConfigFile, "SELL", "over_this", "1.0", strBuf);
	...
	ReadFromIniFile_String(m_strConfigFile, "SELL", "under_this", "1.0", strBuf);
	d = atof((LPSTR)(LPCSTR)strBuf);
	if (d > 30.0) { d = 30.0; }
	strBuf.Format("%.1f", d);
	strBuf.TrimLeft();
	...
	AddMessage("     매도 시점 2. 현재가가 매수 금액의 [%s] 퍼센트 이상 하락시.", strBuf);
...
void CABotDlg::SaveSystemFile()
{
	...
	WriteToIniFile_String(m_strConfigFile, "SELL", "over_this", strBuf);

	// 구매후 종목 현재가가, 이 퍼센트 보다 낮아지면 판다.
	m_cmbSellUnderThis.GetLBText(m_cmbSellUnderThis.GetCurSel(), strBuf);
	WriteToIniFile_String(m_strConfigFile, "SELL", "under_this", strBuf);
}
...
//	theApp.m_khOpenApi.SetInputValue("비밀번호", _T("6452"));
//	theApp.m_khOpenApi.SetInputValue("비밀번호입력매체구분", _T("00"));
//	theApp.m_khOpenApi.SetInputValue("종목코드", _T("039490"));
...
void CABotDlg::OnReceiveTrCondition(LPCTSTR sScrNo, LPCTSTR strCodeList, LPCTSTR strConditionName, long nConditionIndex, long nNext)
{
	...
	if (strConditionName == strCndName)	//현재 조건명과 조회응답으로 들어온 조건명이 같을때만 처리.
	{
		...
		if (strlen(strCodeList)>0)
		{
			...
			{
				strTargetCodeList += m_grdRealAdd.GetItemText(i, 0) + ";";
			}
