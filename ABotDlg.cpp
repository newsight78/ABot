	...
	: CDialogEx(CABotDlg::IDD, pParent)
{
	...
	m_bDoReuseItemAfterTradeDone = FALSE;
...
void CABotDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_CHECK_ITEM_RESUE_AFTER_TRADEDONE, m_checkDoReuseItemAfterTradeDone);
}
...
void CABotDlg::LoadSystemFile()
{
	...
	// 거래했던 종목도 다시 검색되면 거래를 하기.
	ReadFromIniFile_String(m_strConfigFile, "ROUND", "reuse_item_after_trade_done", "0", strBuf);
	m_bDoReuseItemAfterTradeDone = (atoi((LPSTR)(LPCSTR)strBuf) != 0 ? TRUE : FALSE);
	m_checkDoReuseItemAfterTradeDone.SetCheck(m_bDoReuseItemAfterTradeDone ? BST_CHECKED : BST_UNCHECKED);
	AddMessage("     거래했던 종목도 다시 검색되면 거래를 하기 [%s].", m_bDoReuseItemAfterTradeDone ? "함" : "안함");
...
void CABotDlg::SaveSystemFile()
{
	...
	// 거래했던 종목도 다시 검색되면 거래를 하기.
	m_bDoReuseItemAfterTradeDone = (m_checkDoReuseItemAfterTradeDone.GetCheck() == BST_CHECKED);
	strBuf = (m_bDoReuseItemAfterTradeDone ? "1" : "0");
	WriteToIniFile_String(m_strConfigFile, "ROUND", "reuse_item_after_trade_done", strBuf);
...
void CABotDlg::InitProcessCondition()
{
	...
	// 거래했던 종목도 다시 검색되면 거래를 하기.
	m_bDoReuseItemAfterTradeDone = (m_checkDoReuseItemAfterTradeDone.GetCheck() == BST_CHECKED);
	AddMessage(_T("_____::거래했던 종목도 다시 검색되면 거래를 하기 [%s]."), m_bDoReuseItemAfterTradeDone ? "함" : "안함");
...
void CABotDlg::ThreadEnd(void)
{
	...
	while (!CAflThread::Wait(10)
		&& (clock() - startClock)<5000)
	{
...
void CABotDlg::OnReceiveChejanData(LPCTSTR sGubun, long nItemCnt, LPCTSTR sFIdList)
{
	...
	if (nItemIndex >= 0 && strGubun == "0")
	{
		...
		CSingleLock singleLock(&m_criticalItemProcess, TRUE);
		if (singleLock.IsLocked())
		{
#ifdef CRITICAL_LOG //>>>>>>>>>>>>>>>>>>>>>
			AddMessage(_T("Chejan::시작.>>>>>>>>>>>>>>>>>>>>>LOCK_%d"), __LINE__);
#endif//CRITICAL_LOG//<<<<<<<<<<<<<<<<<<<<<
...
#ifdef CRITICAL_LOG //>>>>>>>>>>>>>>>>>>>>>
			AddMessage(_T("Chejan::종료.<<<<<<<<<<<<<<<<<<<<<UNLOCK_%d"), __LINE__);
#endif//CRITICAL_LOG//<<<<<<<<<<<<<<<<<<<<<
			singleLock.Unlock();
		}
	}
	...
	if (0 <= nItemIndex && nItemIndex < _countof(m_Item))
	{
		...
		if (IsInRound() && eST_NONE < aItem.m_eitemState && aItem.m_eitemState < eST_TRADEDONE)
		{
			...
			if (doProcess)
			{
				...
				{
#ifdef CRITICAL_LOG //>>>>>>>>>>>>>>>>>>>>>
					AddMessage(_T("RealData::시작.>>>>>>>>>>>>>>>>>>>>>LOCK_%d"), __LINE__);
#endif//CRITICAL_LOG//<<<<<<<<<<<<<<<<<<<<<
...
#ifdef CRITICAL_LOG //>>>>>>>>>>>>>>>>>>>>>
					AddMessage(_T("RealData::종료.<<<<<<<<<<<<<<<<<<<<<UNLOCK_%d"), __LINE__);
#endif//CRITICAL_LOG//<<<<<<<<<<<<<<<<<<<<<
					singleLock.Unlock();
				}
			}
...
void CABotDlg::OnBnClickedButtonGetCondition()
{
	...
	long lSelectedCondition = m_cmbCondtion.GetCurSel();
	...
	if (m_cmbCondtion.GetCount() > 0)
	{
		...
		{
			m_cmbCondtion.SetCurSel(lSelectedCondition);
		}
	}
}
	...
	if (strName == strConditionName)	//현재 선택된 조건명과 실시간으로 들어온 조건명이 같은지 비교.
	{
		if (sType == "I")	//종목 편입
		{
			...
			if (lRet >= 0)
			{
				...
				if ((IsInRound() && IsInRoundTime() && 0 <= aIndex && aIndex < min(long(_countof(m_Item)), m_nProcessItemCount)) ||
					(!IsInRound() && 0 <= aIndex && aIndex < long(_countof(m_Item))))
				{
					AddMessage("______________________::[%s][%s]종목 운용종목[%3d]에 추가", sCode, strCodeName, aIndex);
					m_Item[aIndex].m_eitemState = eST_ADDED;
					m_Item[aIndex].m_strName = strCodeName;
					m_Item[aIndex].m_strCode = sCode;
					m_mapItemCode[m_Item[aIndex].m_strCode] = aIndex;
					m_mapUsedItemCode[m_Item[aIndex].m_strCode] = aIndex;
				}
			}
		}
...
void CABotDlg::SetControls(BOOL bEnable)
{
	...
	m_checkDoReuseItemAfterTradeDone.EnableWindow(bEnable);
}
...
void CABotDlg::ThreadEntry(void)
{
}

void CABotDlg::ThreadExit(void)
{
}

void CABotDlg::ThreadRun(void)
{
//	MSG msg;
...
void CABotDlg::ProcessTradeAll()
{
	...
	{
#ifdef CRITICAL_LOG //>>>>>>>>>>>>>>>>>>>>>
		AddMessage(_T("ProcessTradeAll::시작.>>>>>>>>>>>>>>>>>>>>>LOCK_%d"), __LINE__);
#endif//CRITICAL_LOG//<<<<<<<<<<<<<<<<<<<<<
		long i = 0;
...
#ifdef CRITICAL_LOG //>>>>>>>>>>>>>>>>>>>>>
		AddMessage(_T("ProcessTradeAll::종료.<<<<<<<<<<<<<<<<<<<<<UNLOCK_%d"), __LINE__);
#endif//CRITICAL_LOG//<<<<<<<<<<<<<<<<<<<<<
		singleLock.Unlock();
	}
}
...
void CABotDlg::ProcessTradeItem(int nItemId, BOOL bFromAllTrade/*=FALSE*/)
{
	...
	case eST_WAITFILTERBUY:
		if (true)
		{
			...
			if (clock >= aItem.m_lfilterBuyAccumTime)
			{
				...
				{
					AddMessage(_T("라운드::[%s][%s][%s], 틱속도[%.2f]틱/초,양거래금액[%.2f]만원/초로 거래를 시작합니다,%s"),
						aItem.m_strCode, aItem.m_strName, aItem.GetStateString(), dtickspeed, dyangcost, bFromAllTrade?"[타이머]":"[이벤트]");
					aItem.m_eitemState = eST_TRYBUY;
					break;
				}

				AddMessage(_T("라운드::[%s][%s][%s], 틱속도[%.2f]틱/초,양거래금액[%.2f]만원/초로 다시 관심 종목에 진입할때까지 거래를 하지 않습니다.%s"),
					aItem.m_strCode, aItem.m_strName, aItem.GetStateString(), dtickspeed, dyangcost, bFromAllTrade ? "[타이머]" : "[이벤트]");
	...
	//	}
		...
		{
			//다시 종목으로 검색될때 까지 안녕~
			AddMessage(_T("라운드::[%s][%s][%s], 잔여 운용 금액 %s[원]이 종목 운용 금액(수수료 포함) %s[원]보다 작습니다. 다시 관심 종목에 진입할때까지 거래를 하지 않습니다.%s"), 
				aItem.m_strCode, aItem.m_strName, aItem.GetStateString(), GetCurrencyString(m_lProcessDR), GetCurrencyString(m_lProcessItemDR), bFromAllTrade ? "[타이머]" : "[이벤트]");
			aItem.m_eitemState = eST_TONONE;
			break;
		}
	...
	//	}
		if (aItem.m_ltryBuyTimeout > 0 && long(GetTickCount()) > aItem.m_ltryBuyTimeout)
		{
			...
			{
				AddMessage(_T("라운드::[%s][%s][%s],단가[%d],수량[%d],재시도[%d], 거래를 포기 합니다.%s"),
					aItem.m_strCode, aItem.m_strName, aItem.GetStateString(), aItem.m_lbuyPrice, aItem.m_lQuantity - aItem.m_lBuyQuantity, aItem.m_ltryBuyCount, bFromAllTrade ? "[타이머]" : "[이벤트]");
				aItem.m_eitemState = eST_BUYCANCEL;
				break;
			}

			AddMessage(_T("라운드::[%s][%s][%s],단가[%d],수량[%d], 매수가 재시도[%d] 되었습니다.%s"),
				aItem.m_strCode, aItem.m_strName, aItem.GetStateString(), aItem.m_lbuyPrice, aItem.m_lQuantity - aItem.m_lBuyQuantity, aItem.m_ltryBuyCount, bFromAllTrade ? "[타이머]" : "[이벤트]");
			aItem.m_eitemState = eST_TRYBUY;
			break;
		}
	...
	case eST_HOLDING:	//보유 상태.
		if (true)//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		{
			...
			AddMessage(_T("라운드::[%s][%s][%s],목표상가[%d],목표하가[%d],수량[%d], 매도에 도전합니다.%s"),
				aItem.m_strCode, aItem.m_strName, aItem.GetStateString(), aItem.m_lsellPrice, aItem.m_lunderPrice, aItem.m_lQuantity, bFromAllTrade ? "[타이머]" : "[이벤트]");
		...
		*/
		...
		{
			AddMessage(_T("라운드::[%s][%s][%s],매도 수량 정정, 최초수량[%d]<==매수수량[%d].%s"),
				aItem.m_strCode, aItem.m_strName, aItem.GetStateString(), aItem.m_lQuantity, aItem.m_lBuyQuantity, bFromAllTrade ? "[타이머]" : "[이벤트]");
			aItem.m_lQuantity = aItem.m_lBuyQuantity;
		}
	...
	case eST_TRYSELL:	//매도 시도 상태.
		...
		//	aItem.m_ltrySellTimeout = 0;
			...
			AddMessage(_T("라운드::[%s][%s][%s],단가[%d],수량[%d], 매도가 시도 되었습니다.%s"),
				aItem.m_strCode, aItem.m_strName, aItem.GetStateString(), aItem.m_lsellPrice, aItem.m_lQuantity - aItem.m_lSellQuantity, bFromAllTrade ? "[타이머]" : "[이벤트]");
			aItem.m_eitemState = eST_WAITSELL;
			break;
		}
		break;
	...
	case eST_WAITSELL:	//매도 체결 대기 상태.
		...
		{
			AddMessage(_T("라운드::[%s][%s][%s],현재가[%d],수량[%d], 라운드 시간 후 시장가 매도 옵션에 의해, 매도가 시장가로 시도합니다.%s"),
				aItem.m_strCode, aItem.m_strName, aItem.GetStateString(), aItem.m_lsellPrice, aItem.m_lQuantity, bFromAllTrade ? "[타이머]" : "[이벤트]");
			aItem.m_eitemState = eST_SELLMARKETVALUE;
			break;
		}
		else if (aItem.m_lcurPrice <= aItem.m_lunderPrice)
		{
			AddMessage(_T("라운드::[%s][%s][%s],현재가▼[%d],수량[%d], 매도가 시장가로 시도합니다. 하락.%s"),
				aItem.m_strCode, aItem.m_strName, aItem.GetStateString(), aItem.m_lsellPrice, aItem.m_lQuantity, bFromAllTrade ? "[타이머]" : "[이벤트]");
			aItem.m_eitemState = eST_SELLMARKETVALUE;
			break;
		}
		else if (aItem.m_ltrySellTimeout0 > 0 && long(GetTickCount()) > aItem.m_ltrySellTimeout0)
		{
			...
			{
				asellPrice = aItem.m_lcurPrice;
				AddMessage(_T("라운드::[%s][%s][%s],현재가[%d][%d],수량[%d], 매도 시간 타임 아웃#0, 현재가로 매도 시도합니다.%s"),
					aItem.m_strCode, aItem.m_strName, aItem.GetStateString(), asellPrice, lestimatedSellprice, aItem.m_lQuantity, bFromAllTrade ? "[타이머]" : "[이벤트]");
			}

			if (aItem.m_lsellPrice != asellPrice || aItem.m_lunderPrice != aunderPrice)
			{
				aItem.m_lunderPrice = aunderPrice;
				AddMessage(_T("라운드::[%s][%s][%s],목표상가[%d],목표하가[%d],수량[%d], 매도 시간 타임 아웃#0, 상가/하가 변경 되었습니다.%s"),
					aItem.m_strCode, aItem.m_strName, aItem.GetStateString(), asellPrice, aItem.m_lunderPrice, aItem.m_lQuantity, bFromAllTrade ? "[타이머]" : "[이벤트]");
		...
		else if (aItem.m_ltrySellTimeout1 > 0 && long(GetTickCount()) > aItem.m_ltrySellTimeout1)
		{
			...
			{
				asellPrice = aItem.m_lcurPrice;
				AddMessage(_T("라운드::[%s][%s][%s],현재가[%d][%d],수량[%d], 매도 시간 타임 아웃#1, 현재가로 매도 시도합니다.%s"),
					aItem.m_strCode, aItem.m_strName, aItem.GetStateString(), asellPrice, lestimatedSellprice, aItem.m_lQuantity, bFromAllTrade ? "[타이머]" : "[이벤트]");
			}
			...
			{
				aItem.m_lunderPrice = aunderPrice;
				AddMessage(_T("라운드::[%s][%s][%s],목표상가[%d],목표하가[%d],수량[%d], 매도 시간 타임 아웃#1, 상가/하가 변경 되었습니다.%s"),
					aItem.m_strCode, aItem.m_strName, aItem.GetStateString(), asellPrice, aItem.m_lunderPrice, aItem.m_lQuantity, bFromAllTrade ? "[타이머]" : "[이벤트]");
	...
	case eST_TRADECLOSING:
		...
		AddMessage(_T("운용 금액 총합은 %s[원]이 되었습니다.%s"), GetCurrencyString(m_lProcessDR), bFromAllTrade ? "[타이머]" : "[이벤트]");
...
//	}
	...
	if (nextQuantity <= 0)	//구매 가능 수량이 0일 경우 예외 처리.
	{
		AddMessage(_T("BUY_ORDER::[%s],[%s][%s][%s],단가[%d],수량[%d], 거래를 포기 합니다.%s"),
			GetOrderTypeString(lOrderType), aItem.m_strCode, aItem.m_strName, aItem.GetStateString(), aItem.m_lbuyPrice, aItem.m_lQuantity - aItem.m_lBuyQuantity, bFromAllTrade ? "[타이머]" : "[이벤트]");
		if (aItem.m_lBuyQuantity > 0)
		{
			//매수 취소.
			if (!REQ_ItemBuyCancel(aItem, bFromAllTrade))
	...
	if (m_bTradeAllowed)
	{
		lRet = theApp.m_khOpenApi.SendOrder(strRQName, m_strScrNo, m_strAccNo,
